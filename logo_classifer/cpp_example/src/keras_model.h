...
public:
  ...
  void show_values() {
    printf("DataChunk2D values(%d,%d,%d):\n", (int)data.size(),(int)data[0].size(),(int)data[0][0].size());
...
public:
  ...
  std::vector<std::vector<std::vector<std::vector<float> > > > m_rowsVec; // cols, rows, depth, kernel
