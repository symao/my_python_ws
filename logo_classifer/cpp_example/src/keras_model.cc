...
void keras::LayerConv2D::load_weights(std::ifstream &fin) {
  ...
  float tmp_float;
  bool skip = false;
  //fin >> m_kernels_cnt >> m_depth >> m_rows >> m_cols >> m_border_mode; // Uncomment for keras 1.x
  fin >> m_rows >> m_cols >> m_depth >> m_kernels_cnt >> padding; // Uncomment for keras 2.x
  if (padding == "[") { padding = "valid"; skip = true; }

  //cout << "LayerConv2D " << m_kernels_cnt << "x" << m_depth << "x" << m_rows <<
  //            "x" << m_cols << " border_mode " << m_border_mode << endl;
   // reading kernel weights for keras 1.x
 ...
 //}
  ...
  // Reading kernel weights for Keras 2.x
  for(int r = 0; r < m_rows; ++r) {
    vector<vector<vector<float> > > tmp_row;
    for(int c = 0; c < m_cols; ++c) {
      vector<vector<float> > tmp_col;
      for(int d = 0; d < m_depth; ++d) {
        if (!skip) { fin >> tmp_char; } // for '['
        else { skip = false; }
        vector<float> tmp_single_depth;
        for(int k = 0; k < m_kernels_cnt; ++k) {
          fin >> tmp_float;
          tmp_single_depth.push_back(tmp_float);
        }
        fin >> tmp_char; // for ']'
        tmp_col.push_back(tmp_single_depth);
      }
      tmp_row.push_back(tmp_col);
    }
    m_rowsVec.push_back(tmp_row);
  }
 
  // Transform to valid type and store to m_kernels to work for keras 2.
  for (int k = 0; k < m_kernels_cnt; ++k){
	  vector<vector<vector<float> > > tmp_depths;
	  ...
	  for (int d = 0; d < m_depth; ++d){
		  ...
		  for (int row = 0; row < m_rows; ++row){
			  ...
			  for (int col = 0; col < m_cols; ++ col){
				  tmp_float = m_rowsVec[row][col][d][k];
				  tmp_row.push_back(tmp_float);
			  }
			  tmp_single_depth.push_back(tmp_row);			  
		  }
...
keras::DataChunk* keras::LayerConv2D::compute_output(keras::DataChunk* dc) {
  ...
  unsigned int st_x = (m_kernels[0][0].size()-1) >> 1; // Uncomment for Keras 1.x
  unsigned int st_y = (m_kernels[0][0][0].size()-1) >> 1; // Uncomment for Keras 1.x
  //unsigned int st_x = (m_rowsVec[0][0].size()-1) >> 1;
  //unsigned int st_y = (m_rowsVec[0][0][0].size()-1) >> 1;
  vector< vector< vector<float> > > y_ret;
  auto const & im = dc->get_3d();

  size_t size_x = (padding == "valid")? im[0].size() - 2 * st_x : im[0].size();
  size_t size_y = (padding == "valid")? im[0][0].size() - 2 * st_y: im[0][0].size();
  ...
  for(unsigned int j = 0; j < m_kernels.size(); ++j) { // loop over kernels
    for(unsigned int m = 0; m < im.size(); ++m) { // loope over image depth
      ...
      vector<vector<float> > tmp_w = (padding == "valid")?
